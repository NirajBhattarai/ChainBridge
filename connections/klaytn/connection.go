...
// Connect starts the Klaytn WS connection
func (c *Connection) Connect() error {
	c.log.Info("Connecting to klaytn chain...", "url", c.endpoint)
	c.log.Info("Connecting to klaytn chain...")
	...
	c.conn = client.NewClient(rpcClient)
	...
	//
	addr := c.kp.Address()
	c.callOpts = &bind.CallOpts{From: common.HexToAddress(addr)}
	return nil
}
...
func (c *Connection) LockAndUpdateOpts() error {
	...
	c.log.Info(head.String())

	// TODO (twiny): does klaytn have a way to get the gas price?
	//
	// if head.BaseFee != nil {
...
func (c *Connection) LatestBlock() (*big.Int, error) {
	c.log.Info("fetching new Header")
